...
uint8_t numero = 123, contador = 0;
int num[3] = {0, 0, 0};
uint8_t Bandera_multiplexado = 0;
...
void __attribute__((picinterrupt(("")))) isr (void){
    if(INTCONbits.RBIF){
        ...
        mostrar(num);
        Bandera_multiplexado++;
        if(Bandera_multiplexado>2){
...
void separacion(uint8_t a, int num[]){
    num[1] = a/100;
    num[2] = (a-(100*(num[1])))/10;
    num[3] = (a-(100*(num[1])+10*(num[2])));
    return;
}
